...
# clock = pygame.time.Clock() #Задаёшь время
pygame.init()
...
fill_color = (32, 52, 71)
# Высчитываем чтоб треугольник оказался в центре экрана
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_RIGHT and rect_x <= screen_width - rect_width - STEP:
                rect_x += STEP
    screen.fill(fill_color)  # Цвет фона в игре
    pygame.draw.rect(screen, rect_color, (rect_x, rect_y, rect_width, rect_height))
    pygame.display.update()
    # clock.tick(1)  # С какой скорость будет менять цвет фона в данном случаии 1 раз в секунду.
